...
async function startDevServer(options) {
  ...
  try {
    ...
    // Start both servers
    await Promise.all([server.start(), devServer.start()]);
    ...
    process.on('SIGINT', async () => {
      ...
      await Promise.all([server.stop(), new Promise((resolve) => devServer.close(resolve))]);
      process.exit(0);
    });
  } catch (error) {
  ...
  return {
    ...
    module: {
      rules: [
        ...
        {
          test: /\.css$/,
          use: isClient ? ['style-loader', 'css-loader'] : ['null-loader'],
        },
        {
